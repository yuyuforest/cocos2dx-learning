...
bool AppDelegate::applicationDidFinishLaunching() {
	...
	// load game resource
	char whistleTotalFrames = 3;
	char whistleFrameName[25];	//不能设为20，否则会stack around the variable 'whistleFrameName' was corrupted
	Animation* whistleAnimation = Animation::create();

	for (int i = 0; i < whistleTotalFrames; i++)
	{
		sprintf(whistleFrameName, "miner-face-whistle-%d.png", i);
		whistleAnimation->addSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName(whistleFrameName));
	}
	whistleAnimation->setDelayPerUnit(0.1);
	AnimationCache::getInstance()->addAnimation(whistleAnimation, "whistleAnimation");

	// load game resource2222222222222222222222222222222222222222
